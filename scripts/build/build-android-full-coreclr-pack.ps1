...
param(
    ...
    [string]$OutputBackendName = "coreclr-full-upstream",
...
if ([string]::IsNullOrWhiteSpace($AndroidSdkLinuxRoot)) {
    $AndroidSdkLinuxRoot = "$wslCacheRoot/android-sdk"
}
if ([string]::IsNullOrWhiteSpace($JavaHomeLinux)) {
    $JavaHomeLinux = "$wslCacheRoot/jdk-21.0.12.1-1"
}
...
$toolchainChecks = @(
    ...
    "$AndroidSdkLinuxRoot/platforms/android-36/android.jar",
    "$AndroidSdkLinuxRoot/build-tools/36.0.0/aapt2",
...
$shellScript = [System.IO.Path]::GetFullPath(
    (Join-Path $PSScriptRoot "build-android-full-coreclr-pack.sh"))
$shellScriptWsl = & wsl.exe -d $Distribution -- wslpath -a ($shellScript -replace '\\', '/')
if ($LASTEXITCODE -ne 0) {
    throw "Could not convert the full CoreCLR pack build script path for WSL."
}
...
if (-not $SkipBuild) {
    & wsl.exe -d $Distribution -- bash $shellScriptWsl `
        ...
        $Configuration
    if ($LASTEXITCODE -ne 0) {
        throw "The full Android CoreCLR pack build failed with exit code $LASTEXITCODE."
    }
}
...
$outputRoot = Join-Path $repositoryRoot `
    "Output\Dependencies\dotnet-runtime\$RuntimeVersion\$OutputBackendName"
...
Write-Host "Built and normalized the full upstream Android CoreCLR runtime pack:"
